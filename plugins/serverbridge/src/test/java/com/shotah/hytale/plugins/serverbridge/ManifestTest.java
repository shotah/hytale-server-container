...
class ManifestTest {
    ...
    void manifestMainClassMatchesActualClass() throws IOException {
        try (InputStream is = getClass().getClassLoader().getResourceAsStream("manifest.json")) {
            ...
            String expectedClass = ServerBridgePlugin.class.getName();
    ...
    @Test
    void manifestNameIsServerBridge() throws IOException {
        try (InputStream is = getClass().getClassLoader().getResourceAsStream("manifest.json")) {
            assertNotNull(is);
            String content = new String(is.readAllBytes(), StandardCharsets.UTF_8);

            assertTrue(content.contains("\"ServerBridge\""), "manifest Name must be ServerBridge");
        }
    }
}
